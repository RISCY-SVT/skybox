...
Env: BUILD_DIR, OUTDIR, CONFIGS, DRIVER_FILTER
USAGE
}
...
ensure_build() {
  if [[ ! -f "$BUILD_DIR/Makefile" ]]; then
    mkdir -p "$BUILD_DIR"
    (cd "$BUILD_DIR" && "${ROOT}/configure" --xlen=32 --osversion=ubuntu/focal)
  fi
  if [[ $DO_BUILD -eq 1 ]]; then
    (cd "$BUILD_DIR" && PATH=/home/svt/tools/verilator/bin:$PATH CC=gcc CXX=g++ make -s)
  fi
}
...
run_case() {
  ...
  for i in $(seq 1 "$repeat"); do
    ...
    echo "==> $name ($driver) run $i: $args" | tee -a "$log"
    set +e
    PATH=/home/svt/tools/verilator/bin:$PATH CONFIGS="$CONFIGS" CC=gcc CXX=g++ \
    ...
    perf=$(grep -E "PERF: instrs=" "$log" | tail -n1 || true)
    elapsed=$(grep -E "Total elapsed time|Elapsed time" "$log" | tail -n1 || true)
  done
...
CASES=(
  ...
  "tri32 rtlsim triangle.cgltrace triangle_ref_32.png 32 32 1 1 2 1"
)
